...
import tp2.tp2.entities.Car;
import tp2.tp2.entities.User;
import tp2.tp2.utils.ItemsGenerator;
  ...
  public static void main(String[] args) {
    ...
    for (User user : users) {
      //System.out.println(user);
    	System.out.println(user.getCar().getCarType().getMark());
    ...
    // 4 : Le nombre d'utilisateur dont le full name contient entre 13 et 16 caractères
    long nbUsersFullname1316 = users.stream().filter(x -> x.getFullName().length() <= 16 || 
    		x.getFullName().length() <= 13).count();
    System.out.println(String.format("%s %d", "Les utilisateurs dont le full name contient entre 13 et 16 caractères",
        nbUsersFullname1316));

    // 5 : Les utilisateurs possédant une voiture de la marque "marque1"
    List<User> usersCarMarque1 = users.stream().filter(x -> x.getCar().getCarType().getMark() == "marque1")
